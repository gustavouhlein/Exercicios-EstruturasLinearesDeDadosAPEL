...
public class DoublyLinkedList<E> implements List<E> {
	...
	public boolean moveToFirst(E element) throws NullPointerException {
		boolean elementoEncontrado = false;
		if(element == null) throw new NullPointerException();

		DNode<E> current = head;
		while (current != null) {
			DNode<E> proximoNo = current.getNext();
			if (element.equals(current.getElement())) {
				elementoEncontrado = true;
				DNode<E> noMovido = current;
				if (noMovido != head)
					noMovido.getPrevious().setNext(noMovido.getNext());
				if(noMovido == tail) 
					tail = noMovido.getPrevious();
				else
					noMovido.getNext().setPrevious(noMovido.getPrevious());
				
				noMovido.setPrevious(null);
				noMovido.setNext(head);
				head.setPrevious(noMovido);
				head = noMovido;
			}
			current = proximoNo;
		}
